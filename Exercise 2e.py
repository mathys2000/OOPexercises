...
#
# Fourth corrected version

# %% [markdown]
# ## Major changes (Exercise 2d)
#
...
# %% [markdown]
# ## Minor improvements (Exercise 2e)
#
# * find_time_overlap: No more in-place change of input list
...
class Oscilloscope:
    ...
    def find_time_overlap(self, signal, t_scale):
        ...
        """
        t0 = max(t_scale[0], signal.tt[0])
        t1 = min(t_scale[1], signal.tt[-1])
        if t1 < t0:
            raise ValueError("No overlap between signal time and scope time_scale")
        ix = np.where(np.logical_and(signal.tt>=t0, signal.tt<=t1))[0]
        return ix
